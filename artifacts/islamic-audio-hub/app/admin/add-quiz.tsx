...
export default function AddQuizScreen() {
  ...
  async function handleSave() {
    if (!question.trim()) {
      Alert.alert('தவறு', 'கேள்வி உள்ளிடுங்க');
      return;
    }
    if (options.some(o => !o.trim())) {
      Alert.alert('தவறு', 'மூன்று விடைகளும் உள்ளிடுங்க');
      return;
    }
    if (correctIndex === null) {
      Alert.alert('தவறு', 'சரியான விடையை தேர்ந்தெடுங்க');
      return;
    }
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      {options.map((opt, i) => (
        <TouchableOpacity
          ...
          style={[
            styles.optionRow,
            correctIndex === i && styles.optionRowCorrect,
          ]}
      ...
      >
        {loading ? (
          ...
          <Text style={styles.saveBtnText}>💾 Quiz சேமி</Text>
        )}
      </TouchableOpacity>
    </ScrollView>
  );
