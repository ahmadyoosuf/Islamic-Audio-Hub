...
export default function EditAudioScreen() {
  ...
  function handleDeleteTrack() {
    Alert.alert(
      'நீக்கவா?',
      `"${title}" audio-ஐ நிரந்தரமாக நீக்க விரும்புகிறீர்களா?`,
      [
  ...
  async function handleSaveNewQuiz() {
    ...
    try {
      ...
      Alert.alert('பிழை', 'Quiz சேமிக்க முடியவில்லை');
    } finally {
      setSavingQuiz(false);
    }
  }
  ...
  async function handleUpdateQuiz(idx: number) {
    ...
    try {
      ...
      Alert.alert('பிழை', 'Quiz update பண்ண முடியவில்லை');
    }
  }
        ...
        <View style={styles.card}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.chipScroll}>
            {CATEGORIES.map(cat => (
              ...
              >
                <Text style={[styles.chipTxt, categoryId === cat.id && { color: cat.color }]}>
                  {cat.name}
                </Text>
              </TouchableOpacity>
            ))}
        ...
        <View style={styles.saveRow}>
          ...
          >
            {saving ? (
              ...
              <Text style={styles.saveBtnTxt}>💾 மாற்றங்களை சேமி</Text>
            )}
          </TouchableOpacity>
              ...
              {!q.isEditing ? (
                ...
                <View>
                  ...
                  <TextInput
                    style={[styles.input, styles.textArea]}
                    value={q.question}
                    onChangeText={v => updateQuizField(qIdx, 'question', v)}
                    placeholderTextColor="#444"
                    multiline
                    numberOfLines={2}
                  />
                  ...
                  {q.options.map((opt, oi) => (
                    <TouchableOpacity
                      key={oi}
                      style={[styles.editOptRow, q.correctIndex === oi && styles.editOptRowCorrect]}
                      onPress={() => updateQuizField(qIdx, 'correctIndex', oi)}
                    >
                      ...
                      <TextInput
                        ...
                        value={opt}
                        onChangeText={v => updateQuizOption(qIdx, oi, v)}
                        placeholderTextColor="#444"
                        placeholder={`விடை ${oi + 1}`}
                      />
                    </TouchableOpacity>
                  ))}
                  <View style={styles.quizActions}>
          ...
          {addingQuiz && (
            <View style={[styles.quizCard, styles.quizCardNew]}>
              ...
              <Text style={styles.fieldLabel}>
                விடைகள் (சரியான விடையை tap பண்ணி select பண்ணுங்க) *
              </Text>
              {newOpts.map((opt, oi) => (
                <TouchableOpacity
                  key={oi}
                  style={[styles.editOptRow, newCorrect === oi && styles.editOptRowCorrect]}
                  onPress={() => setNewCorrect(oi)}
                >
                  ...
                  <TextInput
                    ...
                    value={opt}
                    onChangeText={v => {
                      const copy = [...newOpts] as [string, string, string];
                      copy[oi] = v;
                      setNewOpts(copy);
                    }}
                    placeholderTextColor="#444"
              ...
              <View style={styles.quizActions}>
                <TouchableOpacity
                  style={[styles.quizSaveBtn, savingQuiz && { opacity: 0.6 }]}
